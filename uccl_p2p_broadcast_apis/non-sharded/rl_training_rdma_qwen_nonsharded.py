# rl_training_rdma_qwen_nonsharded.py
"""
RL Training with RDMA Weight Transfer using UCCL P2P (Non-Sharded)
Model: Qwen2.5

NON-SHARDED: Each training rank sends FULL model to ONE inference rank (1-to-1 mapping)
- Rank 0 -> Rank 8
- Rank 1 -> Rank 9
- ...
- Rank 7 -> Rank 15

Comparison with Sharded:
- Sharded: 64 connections (8 senders x 8 receivers), each sends 1/8 model to ALL receivers
- Non-Sharded: 8 connections (1-to-1), each sends FULL model to ONE receiver
"""
...
# ============ PARSE ARGUMENTS ============
parser = argparse.ArgumentParser(description='RL Training with RDMA Weight Transfer (Non-Sharded, Qwen2.5)')
...
# For non-sharded, number of active pairs = min(training, inference)
NUM_ACTIVE_PAIRS = min(NUM_TRAINING_RANKS, NUM_INFERENCE_RANKS)
...
class RolloutGenerator:
    def __init__(self, model, value_head, tokenizer, reward_fn, device, dtype=torch.float32):
        self.model = model
...
class PPOTrainer:
    ...
    def ppo_step(self, batch: RolloutBatch) -> dict:
        ...
        values = self.value_head(hidden_states).float()
...
if rank == 0:
    log("")
    log("=" * 70)
    log("    RL TRAINING WITH RDMA WEIGHT TRANSFER (NON-SHARDED, Qwen2.5)")
    log("=" * 70)
    log(f"  Mode:                NON-SHARDED (1-to-1 mapping)")
    ...
    log(f"  Active Pairs:        {NUM_ACTIVE_PAIRS}")
    log(f"  Connections:         {NUM_ACTIVE_PAIRS} (1-to-1)")
...
# Step 4: RDMA Setup (NON-SHARDED: 1-to-1 mapping)
log("Step 4: RDMA setup (Non-Sharded 1-to-1)")
...
# NON-SHARDED: 1-to-1 mapping
# Training rank i <-> Inference rank (NUM_TRAINING_RANKS + i)
if rank < NUM_ACTIVE_PAIRS:
    # Sender: training rank connects to inference rank
    partner_rank = NUM_TRAINING_RANKS + rank
    
    dist.send(torch.ByteTensor(list(local_metadata)), dst=partner_rank)
    remote_tensor = torch.zeros(len(local_metadata), dtype=torch.uint8)
    dist.recv(remote_tensor, src=partner_rank)
    ...
    if ok:
        connection = conn_id
        log(f"Connected to inference rank {partner_rank}")
    else:
        log(f"ERROR: Failed to connect to rank {partner_rank}")

elif rank in INFERENCE_RANKS and (rank - NUM_TRAINING_RANKS) < NUM_ACTIVE_PAIRS:
    # Receiver: inference rank accepts from training rank
    partner_rank = rank - NUM_TRAINING_RANKS
    ...
    if ok:
        connection = conn_id
        log(f"Accepted connection from training rank {partner_rank}")
    else:
        log(f"ERROR: Failed to accept from rank {partner_rank}")
else:
    log("No RDMA connection (idle rank)")
...
if rank == 0:
    log("")
    log("=" * 70)
    log("                    ROUTING TABLE (NON-SHARDED)")
    log("=" * 70)
    log("")
    log("  1-to-1 Mapping (Each sender sends FULL model to ONE receiver):")
    log("  " + "-" * 60)
    for i in range(NUM_ACTIVE_PAIRS):
        log(f"    Rank {i} [Full Model: {model_size_mb:.1f} MB] ────► Rank {NUM_TRAINING_RANKS + i}")
    log("  " + "-" * 60)
    log(f"    Total Connections:   {NUM_ACTIVE_PAIRS}")
    log(f"    Data per Sender:     {model_size_mb:.2f} MB (full model)")
    log(f"    Data per Receiver:   {model_size_mb:.2f} MB (full model)")
    log(f"    Total Data Moved:    {model_size_mb * NUM_ACTIVE_PAIRS:.2f} MB")
    if NUM_TRAINING_RANKS > NUM_ACTIVE_PAIRS:
        log(f"    Idle Training:       Ranks {NUM_ACTIVE_PAIRS}-{NUM_TRAINING_RANKS-1}")
    if NUM_INFERENCE_RANKS > NUM_ACTIVE_PAIRS:
        log(f"    Idle Inference:      Ranks {NUM_TRAINING_RANKS + NUM_ACTIVE_PAIRS}-{NUM_TRAINING_RANKS + NUM_INFERENCE_RANKS - 1}")
    log("=" * 70)
...
for rl_iter in range(RL_ITERATIONS):
    ...
    if role == "training":
        ...
        iter_reward = rollouts.rewards.mean().item()
        
        if rank == 0:
            log(f"  Time: {ppo_time:.2f}s | Reward: {iter_reward:.4f}")
    else:
        log("Waiting for training...")
    
    dist.barrier()
    
    # PHASE 2: RDMA Transfer (NON-SHARDED: 1-to-1)
    if rank == 0:
        log("")
        log("PHASE 2: RDMA WEIGHT TRANSFER (1-to-1)")
        log("-" * 50)
    
    transfer_start = time.perf_counter()
    
    # Sender: training rank with connection
    if rank < NUM_ACTIVE_PAIRS and connection is not None:
        # Get model parameters
        if isinstance(model, DDP):
            params_iter = model.module.named_parameters()
        else:
        ...
        for name, param in params_list:
            ...
            if ok:
                registered_mrs.append((name, mr_id, tensor.data_ptr(), tensor.numel() * tensor.element_size()))
        ...
        # Send FULL model
        for name, mr_id, ptr, size in registered_mrs:
            ok = ep.send(connection, mr_id, ptr, size)
            if ok:
                total_bytes += size
        ...
        log(f"  Sent {total_bytes/1e6:.1f} MB to Rank {partner_rank} | {duration:.4f}s | {bw:.2f} GB/s")
    
    # Receiver: inference rank with connection
    elif rank in INFERENCE_RANKS and connection is not None:
        ...
        # Receive FULL model
        for name, mr_id, ptr, size in registered_mrs:
            ok = ep.recv(connection, mr_id, ptr, size)
            if ok:
                total_bytes += size
        ...
        duration = time.perf_counter() - t0
        bw = (total_bytes / 1e9) / duration if duration > 0 else 0
        log(f"  Received {total_bytes/1e6:.1f} MB from Rank {partner_rank} | {duration:.4f}s | {bw:.2f} GB/s")
    
    else:
        torch.cuda.synchronize()
        dist.barrier()
        log("  Idle (no RDMA transfer)")
    ...
    if rank == 0:
        # Aggregate BW for non-sharded = N_pairs * model_size / time
        aggregate_bw = (model_size_mb * NUM_ACTIVE_PAIRS / 1e3) / transfer_time if transfer_time > 0 else 0
        iter_bw = aggregate_bw
        log(f"  Total: {transfer_time:.4f}s | Aggregate BW: {aggregate_bw:.2f} GB/s")
    ...
    if rank == 0:
        ...
        log(f"  │  Aggregate BW:       {iter_bw:>8.2f} GB/s{' '*13}│")
...
if rank == 0:
    ...
    total_training = sum(m['training_time'] for m in iter_metrics)
    total_transfer = sum(m['transfer_time'] for m in iter_metrics)
    
    log(f"  │ {'AVG':^4} │ {avg_training:^10.2f} │ {avg_transfer:^10.4f} │ {avg_bw:^10.2f} │ {avg_reward:^8.4f} │")
    log("  " + "─" * 66)
    
    log("")
    log("  CONFIGURATION (NON-SHARDED):")
    log("  " + "-" * 50)
    log(f"    Mode:                Non-Sharded (1-to-1)")
    log(f"    Model:               {MODEL_NAME} ({model_size_mb:.2f} MB)")
    log(f"    Dtype:               {args.dtype}")
    log(f"    Training GPUs:       {NUM_TRAINING_RANKS}")
    log(f"    Inference GPUs:      {NUM_INFERENCE_RANKS}")
    log(f"    Active Pairs:        {NUM_ACTIVE_PAIRS}")
    log(f"    Connections:         {NUM_ACTIVE_PAIRS}")
    log(f"    Total Training:      {total_training:.2f}s")
    log(f"    Total Transfer:      {total_transfer:.4f}s")
    log(f"    Transfer Overhead:   {100 * total_transfer / (total_training + total_transfer + 0.001):.2f}%")
    log("  " + "-" * 50)
    log("=" * 70)
