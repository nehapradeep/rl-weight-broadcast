# rl_training_rdma_gpt2_nonsharded.py
"""
...
# ============ PARSE ARGUMENTS ============
parser = argparse.ArgumentParser(description='RL Training with RDMA Weight Transfer (Non-Sharded, GPT-2)')
...
if rank == 0:
    log("")
    log("=" * 70)
    log("     RL TRAINING WITH RDMA WEIGHT TRANSFER (NON-SHARDED, GPT-2)")
    log("=" * 70)
...
try:
    ...
    log(f"GPU Memory: {allocated:.2f} GB allocated")
...
if rank == 0:
    ...
    log(f"Hidden size: {config.n_embd}")
    log(f"Num layers: {config.n_layer}")
...
if rank == 0:
    ...
    log(f"    Model:               GPT-2 ({model_size_mb:.2f} MB)")
