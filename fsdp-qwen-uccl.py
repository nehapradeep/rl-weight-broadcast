...
def broadcast_model_to_inference(model, rank, bridge_group, is_sender=False):
    ...
    # If sender (Rank 0), 'model' is likely a CPU copy from FSDP gather
    # If receiver (Rank 16), 'model' is a standard GPU model
    ...
    for name, tensor in items:
        # 1. Move to GPU if needed (UCCL RDMA requirement)
        ...
        # 3. Register (Pin) memory for zero-copy RDMA
        ...
        if is_sender:
            ...
            # Copy back logic for receiver if needed
            # (Usually not needed if model was already on GPU, but safe to check)
            if not tensor.is_cuda:
    ...
    if is_sender:
        ...
        logging.info(f"UCCL Receive complete. Time: {duration:.4f}s")
        sys.stdout.flush()
...
def run_trainer(rank, world_size, train_group, bridge_group):
    ...
    # Pass the NCCL train_group to FSDP for training comms
    model = FSDP(
        model,
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        if rank == 0:
            logging.info("Gathering complete. Broadcasting via UCCL...")
            # Load into a temporary standard model for broadcasting
...
# -----------------------------------------------------------
def main():
    # Sync Interfaces: Copy NCCL to GLOO if needed
    ...
    # 1. Init GLOO (Required for UCCL Control Plane)
    ...
    # Bridge group for logical grouping, but actual transfer uses UCCL
    bridge_group = dist.new_group(ranks=bridge_ranks, backend="gloo")

    if rank in train_ranks:
        run_trainer(rank, TRAIN_WORLD_SIZE, train_group, bridge_group)
