    ...
    def compute_routing_tables(
        ...
        for param_name, train_ranks, infer_ranks in matched_params:
            ...
            for infer_rank in infer_ranks:
                ...
                # Compute bytes for this single transfer
                element_size = torch.tensor([], dtype=param_meta.dtype).element_size()
                num_elements = 1
                for dim in param_meta.shape:
                    num_elements *= dim
                transfer_bytes = num_elements * element_size

                # Update load balancing tracker
                bytes_sent[src_rank] += transfer_bytes
